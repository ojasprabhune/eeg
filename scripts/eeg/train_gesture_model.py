"""
Script to train the gesture model on the gesture2hand dataset. The model is a
transformer encoder + decoder architecture that takes in one EEG epoch (one
letter's motor execution window, either bandpower or raw channels) and
predicts the corresponding gesture class for that epoch.
"""
...
def validate() -> tuple[float, float, float, torch.Tensor]:
    ...
    val_confusion_matrix = torch.zeros(
        train_dataset.num_classes, train_dataset.num_classes, dtype=int32
    )
...
def train():
    ...
    for i in epoch_tqdm:
        ...
        for raw, bp, labels in train_loader:
