...
REP_PENALTY = float(
    os.environ.get("CORPUS_REP_PENALTY", 4.0)
)  # per-repeat logit penalty
...
def generate_corpus(model, tokenizer, experiment: str) -> list[str]:
    ...
    for _ in range(MAX_PASSES):
        if len(sentences) >= NUM_SENTENCES:
            break
        raw = generate_pass(
            model, prompt_ids, trie.root, transitions, tok_text, temperature
        )
        new_count = 0
        for sentence in raw:
    ...
    if conn and container:
        ...
        for path in written:
            ...
            with open(path, "rb") as data:
                client.get_blob_client(container, name).upload_blob(
                    data, overwrite=True
                )
