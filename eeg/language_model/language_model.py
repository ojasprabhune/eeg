...
class LanguageModel(nn.Module):
    ...
    def forward(
        ...
        # only mix in self-predictions during training. at validation/inference
        # tgt is either ground truth (teacher-forced eval) or the model's own
        # already-committed autoregressive prefix, and in both cases we want a
        # single clean decoder pass with no bernoulli coin flips swapping tokens
        if use_scheduled_sampling:
            ...
            epsilon = 0.0  # no mixing happened, neutral value if it's ever logged
